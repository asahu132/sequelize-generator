...
var Filter = /** @class */ (function () {
    function Filter(queryGroup, sortBy, model, alias, include) {
        this.context = {};
        ...
        this.include = include ? include.map(function (include) { return Filter.build(include); }) : [];
    }
    Filter.merge = function (filters) {
        var returnedFilter = filters[0].clone();
        if (returnedFilter.queryGroup.length) {
            returnedFilter.queryGroup = [new query_1.Query(returnedFilter.queryGroupMergeOperator, returnedFilter.queryGroup)];
        }
    ...
    Filter.prototype.unsequelize = function (sequelized) {
        ...
        if (Object.keys(sequelized).length > 0) {
            ...
            if (sequelized.include && sequelized.include.length > 0) {
                sequelized.include.forEach(function (include) {
                    _this.include.push(new Filter().unsequelize(include));
                });
            }
        }
        return this;
    ...
    Filter.prototype.sequelize = function () {
        ...
        if (this.include && this.include.length > 0) {
            sequalize['include'] = [];
            var include = this.include.map(function (include) {
    ...
    Filter.prototype.clone = function () {
        return new Filter(this.queryGroup.map(function (query) { return query.clone(); }), this.sortBy.map(function (sort) { return sort.clone(); }), this.model, this.alias, this.include.map(function (include) { return include.clone(); }));
    };
    ...
    Filter.prototype.removeRule = function (rule) {
        var _this = this;
        if (rule) {
            this.queryGroup.forEach(function (queryGroupEntry, index) {
                queryGroupEntry.removeRule(rule);
                if (!queryGroupEntry.rules.length) {
                    _this.queryGroup.splice(index, 1);
                }
            });
        }
    };
